...
public class ShootingPhoneWorldUnits extends ApplicationAdapter {
	SpriteBatch batch;
	...
	public void create () {
		...
		// create fit-view port
		fitViewport = new FitViewport(500, 400);
		...
		phone.x = (int) (fitViewport.getWorldWidth() / 2f - phoneImage.getWidth() / 2f);
		phone.y = 20;
		// setting dimensions
	...
	public void render () {
		...
		fitViewport.apply();
		// render sprite batch in the coordinate system specified by the fit view port
		...
		{
			// draw background
			batch.draw(backgroundImage, 0, 0, fitViewport.getWorldWidth(), fitViewport.getWorldHeight());
			...
			font.draw(batch, "" + filesDownloaded, fitViewport.getWorldWidth() - 60, fitViewport.getWorldHeight() - 20);

			// display memory info
			if (memoryCapacity >= 10) {
				font.setColor(Color.RED);
				font.draw(batch, "MEMORY FULL", fitViewport.getWorldWidth() / 2f - 30, fitViewport.getWorldHeight() / 2f);
			}
			...
			batch.draw(memoryRotatedImage, 13, fitViewport.getWorldHeight() - 125);
			font.draw(batch, "" + memoryCapacity, 90, fitViewport.getWorldHeight() - 83);

			// draw battery
			if (batteryLife >= 67) {
				batch.draw(batteryFullImage, 13, fitViewport.getWorldHeight() - 70);
				font.setColor(Color.GREEN);
			}
			else if (batteryLife >= 33) {
				batch.draw(batteryHalfImage, 13, fitViewport.getWorldHeight() - 70);
				font.setColor(Color.YELLOW);

			}
			else if (batteryLife >= 10) {
				batch.draw(batteryLowImage, 13, fitViewport.getWorldHeight() - 70);
				font.setColor(Color.RED);
			}
			else if (batteryLife > 0) {
				batch.draw(batteryVeryLowImage, 13, fitViewport.getWorldHeight() - 70);
				font.setColor(Color.RED);
			}
			else if (batteryLife == 0) {
				batch.draw(batteryDeadImage, 13, fitViewport.getWorldHeight() - 70);
				backgroundMusic.stop();
				font.setColor(Color.RED);
				font.draw(batch, "GAME OVER", fitViewport.getWorldWidth() / 2f - 20, fitViewport.getWorldHeight() / 2f);
			}
			// draw battery life number
			font.draw(batch, "" + batteryLife, 90, fitViewport.getWorldHeight() - 25);
		}
		batch.end();
	}
	...
	private void spawnVirus() {
		...
		virus.x = MathUtils.random(0, fitViewport.getWorldWidth() - virusImage.getWidth());
		virus.y = fitViewport.getWorldHeight();
	...
	private void spawnFile() {
		...
		file.x = MathUtils.random(0, fitViewport.getWorldWidth() - fileImage.getWidth());
		file.y = fitViewport.getWorldHeight();
	...
	private void spawnImgFile() {
		Rectangle file = new Rectangle();
		file.x = MathUtils.random(0, fitViewport.getWorldWidth() - imageFileImage.getWidth());
		file.y = fitViewport.getWorldHeight();
	...
	private void spawnVideoFile() {
		Rectangle file = new Rectangle();
		file.x = MathUtils.random(0, fitViewport.getWorldWidth() - videoFileImage.getWidth());
		file.y = fitViewport.getWorldHeight();
	...
	private void spawnMemory() {
		...
		memstick.x = MathUtils.random(0, fitViewport.getWorldWidth() - memoryImage.getWidth());
		memstick.y = fitViewport.getWorldHeight();
	...
	private void spawnCharge() {
		...
		charge.x = MathUtils.random(0, fitViewport.getWorldWidth() - chargeImage.getWidth());
		charge.y = fitViewport.getWorldHeight();
	...
	private void moveRight() {
		phone.x += SPEED * Gdx.graphics.getDeltaTime();
		if (phone.x > fitViewport.getWorldWidth() - phoneImage.getWidth())
			phone.x = fitViewport.getWorldWidth() - phoneImage.getWidth();
	}
